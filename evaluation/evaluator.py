...
class FullEvaluator:
    ...
    def evaluate_model(self, model, test_data_path: str, corpus_path: str,
        ...
        # Static mask Option B: bloom_mask_head with bloom_logit (no shared_mlp).
        # For each level b, corpus is pre-masked with mask_b and both sides normalized
        # within the masked subspace — matching training (masked_q · masked_doc).
        use_static_mask = (
        ...
        if use_prefix_slicing:
            ...
            # Option B (static scattered mask): per-level masked corpus retrieval.
            # For level b: sim = normalize(q * mask_b) · normalize(corpus * mask_b)
            # Matches training where both query and document are masked with the same
            # static level mask, keeping contrastive loss within the masked subspace.
