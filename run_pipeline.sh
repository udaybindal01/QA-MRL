...
MRL_CONT_CKPT="/tmp/mrl-continued-ckpts"
...
# ── Step 4a: Train MRL-continued (fair baseline: same total training as BAM) ──
# This gives reviewers a training-budget-matched MRL to compare against BAM.
log "=== STEP 4a/7: Training MRL-continued baseline (15 more epochs from best MRL) ==="
python scripts/train_baseline_mrl.py \
    --config configs/neurips.yaml \
    --resume "$MRL_CKPT/best/" \
    --checkpoint_dir "$MRL_CONT_CKPT" \
    2>&1 | tee "$LOG_DIR/train_mrl_continued.log"

log "=== STEP 4b/7: Selecting best MRL-continued checkpoint ==="
python scripts/find_best_epoch.py \
    --config configs/neurips.yaml \
    --checkpoint_dir "$MRL_CONT_CKPT" \
    --model_type mrl \
    --metric recall@10 \
    2>&1 | tee "$LOG_DIR/find_best_mrl_continued.log"
log "Best MRL-continued checkpoint: $MRL_CONT_CKPT/best/"

# ── Step 4c: Train BAM (warm-start from best MRL checkpoint) ──────────────────
if [ "$SKIP_BAM" = false ]; then
  log "=== STEP 4c/7: Training BAM (init from $MRL_CKPT/best/) ==="
  python scripts/train_bam.py \
      --config configs/bam.yaml \
      --init_encoder "$MRL_CKPT/best/" \
      2>&1 | tee "$LOG_DIR/train_bam.log"
  log "Step 4c done. Checkpoints at: $BAM_CKPT"
else
...
if [ "$SKIP_EVAL" = false ]; then
  ...
  log "  6a: BAM vs MRL comparison (including MRL-continued fair baseline)..."
  python scripts/eval_bam.py \
      --config        configs/bam.yaml \
      --checkpoint    "$BAM_CKPT/best/" \
      --baseline      "$MRL_CKPT/best/" \
      --mrl_continued "$MRL_CONT_CKPT/best/" \
...
log "  MRL-continued : $MRL_CONT_CKPT/best/"
